...
\end{equation}
\\By combining the results from the SuperK which uses exclusively equation \ref{neutrino_scattering} and SNO results for CC results using equation \ref{charged-current_reaction} it is possible to solve the solar neutrino problem. SuperK found 45\,$\%$ of the expected number of all types of neutrinos according to the standard solar model (need to cite superK paper here) . Whereas the CC results from SNO found 35\,\% of the $\nu_e$ expected (cite sno paper), so there is a 10\,\% discrepancy between these two results. But this does not mean 10\,\% of the neutrinos were $\nu_\mu$ and $\nu_\tau$, as the SuperK detector is 6.5 times more sensitive to $\nu_e$ so 10\,\% $\times$ 6.5 = 65\,\% of the neutrinos are $\nu_\mu$ and $\nu_\tau$. And so all neutrinos are now accounted for and thus the solar neutrino problem was deemed solved \cite{griffiths2008book} \cite{griffiths2008neutrinoOscillations}.
